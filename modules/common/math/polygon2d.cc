...
double Polygon2d::DistanceTo(const Polygon2d &polygon) const {
  CHECK_GE(points_.size(), 3);// 确保第一个多边形顶点数大于3
  CHECK_GE(polygon.num_points(), 3);// 确保第二个多边形顶点数大于3
  ...
  double distance = std::numeric_limits<double>::infinity();
  // 求第二个多边形的每个顶点到第一个多边形每一条边的距离，并返回这些距离的最小值，即是两个多边形的最小间距
  for (int i = 0; i < num_points_; ++i) {
...
bool Polygon2d::ComputeConvexHull(const std::vector<Vec2d> &points,
  ...
  // 逆时针排列所有的顶点（从左到右，从下到上），排列的结果是顶点的id号，仍然存储在sorted_indices
  ...
  // 至此，sorted_indices中是按照逆时针顺序排列的顶点的id号
  int count = 0;
  std::vector<int> results;
  // reserve用来（预留空间，）改变capacity，不改变size，会去分配内存，但不会构造出对象
  results.reserve(n);
  int last_count = 1;
  // 判断是否为凸多边形，并将凸多边形的顶点id号存储在result中
  for (int i = 0; i < n + n; ++i) {
    if (i == n) {
      last_count = count;
    }
	...
	// 因为所有顶点已经按照逆时针顺序排列好，所以读于任意的一个点p(i),都有p(i+2),在向量p(i)p(i+1)的左边,也就是说
	// 向量p(i)p(i+1)与向量p(i)p(i+2)的向量积应该大于0，如果小于0,说明不是凸多变形,且凹点为向量p(i+1)
    while (count > last_count &&
           CrossProd(points[results[count - 2]], points[results[count - 1]],
                     pt) <= kMathEpsilon) {
      results.pop_back();// 去除凹点
      --count;
    }
