...
bool ReferenceLineProvider::GetReferenceLines(
  ...
  if (FLAGS_use_navigation_mode) {
  	// 获取当前时间戳
    double start_time = Clock::NowInSeconds();
	// 从相对地图获取reference_lines和segments
  ...
  if (FLAGS_enable_reference_line_provider_thread) {
    std::lock_guard<std::mutex> lock(reference_lines_mutex_);
	// 如果指引线集合不为空,说明能够从reference_line_provider_线程中获得了最新的指引线集合
...
bool ReferenceLineProvider::GetReferenceLinesFromRelativeMap(
  ...
  hdmap::LaneWaypoint adc_lane_way_point;//无人车所在的路径点(其实就是距离无人车最近的路径点)
