...
// EM　规划器的入口, 规划起点planning_start_point实际上是缝合轨迹的最后一个点
Status EMPlanner::Plan(const TrajectoryPoint& planning_start_point,
  ...
  for (auto& reference_line_info : frame->reference_line_info()) {
	...
	// 如果基于当前循环的reference_line_info_规划成功，且reference_line_info_本身是IsDriveable()
...
Status EMPlanner::PlanOnReferenceLine(
  ...
  }// 如果不是换道参考线,那么要将该条参考线增加直行cost
  ADEBUG << "planning start point:" << planning_start_point.DebugString();

  // 取出reference_line_info中需要被赋值的数据heuristic_speed_data
  auto* heuristic_speed_data = reference_line_info->mutable_speed_data();
  // 利用上一周期的reference_line_info的speed_profile求取本周期reference_line_info的speed_profile
  auto speed_profile =
      GenerateInitSpeedProfile(planning_start_point, reference_line_info);
  // 如果上面求取speed_profile不成功(比如规划的第一个周期),就按照下面的方法求取
  ...
  // speed_profile赋值到本周期的reference_line_info->mutable_speed_data()
...
std::vector<SpeedPoint> EMPlanner::GenerateInitSpeedProfile(
  ...
  // 获取上一周期的最终发布的reference_line_info
  ...
  // 判断当前的reference_line_info是不是起始于last_reference_line_info
  ...
  // 获取last_reference_line_info的速度数据
  ...
  if (!last_speed_vector.empty()) {
	...
	// 上一周期规划起始点换到sl坐标(将点投影到referen_line_info上,求出s,l)
