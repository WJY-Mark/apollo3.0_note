...
Frame::Frame(uint32_t sequence_num,
  ...
  if (FLAGS_enable_lag_prediction) {
    lag_predictor_.reset(
        new LagPrediction(FLAGS_lag_prediction_min_appear_num,//5
                          FLAGS_lag_prediction_max_disappear_num));//3
  }
}
...
bool Frame::CreateReferenceLineInfo() {
  ...
  // 获取参考线集合列表reference_lines和路段集合列表segments
...
Status Frame::Init() {
  ...
  if (FLAGS_enable_prediction && AdapterManager::GetPrediction() &&
	...
	else {// 不启用滞后预测功能，直接获取预测模块最新一次发布的障碍物预测信息
  ...
  // collision_obstacle为空，表示没有撞，不为空，相撞
...
const Obstacle *Frame::FindCollisionObstacle() const {
  ...
  Vec2d position(vehicle_state_.x(), vehicle_state_.y());//车辆位置(车辆后轴中心位置)
  ...
  for (const auto &obstacle : obstacles_.Items()) {
    ...
    // 车辆包络矩形中心到障碍物包络矩形中心的距离
    double center_dist =
        adc_box.center().DistanceTo(obstacle->PerceptionBoundingBox().center());
	// 如果两个包络矩形的中心距离大于两个包络矩形半对角线长+碰撞阈值，那么该障碍物不会与车辆碰撞，考虑下一障碍物
	...
	// 运行到这里说明不满足上一个if的条件，可能相撞，需要进行下面的计算来进一步确认
	// 计算障碍物的凸多边形轮廓到车辆包络矩形轮廓的最小距离
	...
	// 如果两个多边形(障碍物和车辆)的最小间距小于阈值,那么二者相撞,返回相撞的障碍物
...
const ReferenceLineInfo *Frame::FindDriveReferenceLineInfo() {
  // 从所有可行使的reference_line_info_中找出最好的(cost最小的)reference_line_info_
